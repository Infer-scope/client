...
class StoredArtifactHelper:
    ...
    def __init__(self, client: Client, parent_project_uid: Union[UUID, None] = None):
        self._client = client
        self._parent_project_uid = parent_project_uid if parent_project_uid is not None else client.get_default_project().uid
    ...
    def upload_artifact(
        ...
        assert bool(blob) != bool(local_path), "Exactly one parameter: blob or local_path can be specified"
        if local_path:
